---
name: add-node-type
description: Add a new AST node type (JavaScript, JSX, or TypeScript) to @eslint/jsparse. Use when adding, registering, or implementing a node kind, ESTree node, or syntax the parser does not yet support, or when a node parses but something downstream ignores it. Covers node-kinds, slots, the parser, the decoder, ast-types, and jsscope, and verifies the result with a driver script.
---

# Adding a node type

A node kind has to be registered in **seven places across two packages**. Six
of them fail loudly. One does not: miss the `jsscope` slot-names entry and the
binary walk keeps working while the tree walk silently stops descending into
the node.

`check-node-kind.mjs` checks all seven at once and then proves the node works
by running it through both packages. Use it after every step; it is faster than
...
sample, decodes it, validates it in both dialects, runs **both** `jsscope`
entry points and compares the graphs, and diffs the node against `espree` or
...
## Registration sites
  ...
  ok    kind constant      packages/jsparse/src/node-kinds.ts
  ok    kind name          packages/jsparse/src/node-kinds.ts
  ok    slot layout        packages/jsparse/src/slots.ts
  ok    parser emits it    packages/jsparse/src/parser.ts
  ok    decoder case       packages/jsparse/src/to-ast.ts
  ok    type declaration   packages/jsparse/src/ast-types.ts
  ok    scope slot names   packages/jsscope/src/slot-names.ts
...
## Round trip
  ...
  ok    jsscope: both entry points produce the same graph
...
**1. `packages/jsparse/src/node-kinds.ts`** — append the constant in the right
...
**2. `packages/jsparse/src/slots.ts`** — add the kind to a `define()` whose
...
**4. `packages/jsparse/src/to-ast.ts`** — a `case` in `fill()` for JavaScript
...
**5. `packages/jsparse/src/ast-types.ts`** — the interface. Get the scaffold
from the generator rather than writing it blind:

```bash
node packages/jsparse/scripts/derive-shapes.mjs --list
```
...
**6. `packages/jsscope/src/slot-names.ts`** — what each child slot is *called*
...
**7. `packages/jsparse/tests/fixtures/`** — a snippet in `javascript.json`,
...
`packages/jsparse/scripts/conformance-types.mjs` instead.
...
`npm run conformance --workspace=@eslint/jsscope` when the node has children;
...
- **A missing `slot-names.ts` entry changes nothing you can see by hand.** The
  ...
  comparison or the jsscope corpus run finds it.
- **The two scope entry points do not represent nodes the same way.** The
  ...
  uses `packages/jsscope/scripts/serialize.mjs` — the project's own
...
| `FAIL jsscope entry points disagree` | Missing or wrong `slot-names.ts` entry — the names must match the ESTree property names exactly. |
...
- [`packages/jsparse/docs/types.md`](../../../packages/jsparse/docs/types.md)
