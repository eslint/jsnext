...
- **Most of ECMAScript's early errors.** Around 26 test262 files are invalid
  programs that both phases accept: `new.target` outside a function, a line
  terminator between `yield` and its `*`, `delete x` in strict mode.
