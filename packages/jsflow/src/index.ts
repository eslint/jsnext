 ...
 *
 * `createGraph()` reads `@eslint/jsparse`'s binary AST and
 ...
 * into the AST buffer, and every variable write points at its reference
 ...
 * @param ast The AST buffer returned by `@eslint/jsparse`'s `parse()`.
