/**
 * @fileoverview Compares jsparse against the parsers it aims to replace.
 *
 ...
 *
 * jsparse appears three times in the AST tier because its phases are
...
async function contenders(dialect) {
	...
	/*
	 * `dialect: "jsx"` is not a thing jsparse is told about. JSX is JavaScript
	...
	const list = [
		{
			...
			tier: AST,
			run: code => jsparse.parse(code),
		},
		{
			key: "jsparse-validate",
			name: "jsparse: parse() + validate()",
			note: "buffers plus every context-dependent diagnostic",
			tier: AST,
			run: code => jsparse.validate(jsparse.parse(code), jsparseOptions),
		},
		{
			key: "jsparse-to-ast",
			name: "jsparse: parse() + toAST()",
			tier: AST,
			run: code => jsparse.toAST(jsparse.parse(code), jsparseOptions),
		},
		{
			key: "jsparse-eslint",
			name: "jsparse: eslintParser.parse()",
			tier: ESLINT,
			run: code =>
