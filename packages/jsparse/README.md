...
under this workspace's `node_modules` with both `jsparse` and the reference
parser and compares the results structurally. As of this writing that is 1424
JavaScript files (ASTs, tokens, and comments) and 1185 TypeScript files, with
zero differences. JSX has no real-world corpus to point it at, so it is
covered by the fixture suite instead: every sample in
`tests/fixtures/jsx.json` is compared against both `espree` and
...
npm run lint      # lint this repository with its own parser (root only)
