...
also why text has to exist somewhere. When `AstReader#text()` or `jsscope`'s
`BinaryAst#name()` needs to know what an identifier is called, it slices the
characters out of the source text using the record's two offsets:
...
module.** `@eslint/jsscope` bundles its own copy of the reader, so a buffer
produced by `@eslint/jsparse` missed the `WeakMap` living inside
`@eslint/jsscope` and looked exactly like a buffer from another process. The
plain `analyze(parse(code))` case threw.
