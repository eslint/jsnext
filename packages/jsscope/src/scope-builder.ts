...
export class ScopeBuilder<TNode> {
	...
	finish(treeHandles: boolean): ArrayBuffer {
		...
		out[H_MAGIC] = SCOPE_BUFFER_MAGIC;
		out[H_VERSION] = SCOPE_BUFFER_VERSION;
		out[H_FLAGS] = treeHandles ? BUFFER_TREE_HANDLES : 0;
		out[H_SCOPE_COUNT] = this.scopeCount;
		out[H_SYMBOL_COUNT] = finalSymbols.length;
		out[H_REFERENCE_COUNT] = this.refCount;
		out[H_DEFINITION_COUNT] = finalDefinitions.length;
		out[H_SCOPES_BASE] = scopesBase;
		out[H_SYMBOLS_BASE] = symbolsBase;
		out[H_REFERENCES_BASE] = referencesBase;
		out[H_DEFINITIONS_BASE] = definitionsBase;
		out[H_POOL_BASE] = poolBase;
		out[H_NODE_SCOPE_BASE] = nodeScopeBase;
		out[H_NODE_SCOPE_COUNT] = nodeScopeWords.length / 2;
		out[H_DECLARED_BASE] = declaredBase;
		out[H_DECLARED_COUNT] = declaredWords.length / 2;
		out[H_IDENT_REF_BASE] = identRefBase;
		out[H_IDENT_REF_COUNT] = identRefWords.length / 2;
		out[H_STRINGS_BASE] = stringsBase;
		out[H_STRING_COUNT] = this.strings.length;
		out[H_STRING_BYTES] = byteLength;
		out[H_OPTIONS] =
		...
		out[H_JSX_PRAGMA] = jsxPragmaId;
		out[H_JSX_FRAGMENT] = jsxFragmentId;
