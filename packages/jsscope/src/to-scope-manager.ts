...
import { N_ArrowFunctionExpression } from "@eslint/jsparse";
...
import type { AstReader, ParseResult } from "@eslint/jsparse";
...
function readOptions(reader: ScopeBufferReader): ResolvedOptions {
	const stored = reader.words[H_OPTIONS];
	const sourceTypeCode = stored & OPT_SOURCE_TYPE_MASK;
	const pragma = reader.words[H_JSX_PRAGMA];
	const fragment = reader.words[H_JSX_FRAGMENT];
...
function rehydrate<TNode>(
	...
	const declaredBase = buffer.words[H_DECLARED_BASE];
	const declaredCount = buffer.words[H_DECLARED_COUNT];
