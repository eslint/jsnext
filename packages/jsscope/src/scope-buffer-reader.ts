...
export class ScopeBufferReader {
	 ...
	 * @throws {TypeError} When the buffer is not a jsscope scope buffer.
	 */
	constructor(buffer: ArrayBufferLike) {
		...
		if (
			words.length < 2 ||
			words[H_MAGIC] !== SCOPE_BUFFER_MAGIC ||
			words[H_VERSION] !== SCOPE_BUFFER_VERSION
		) {
			throw new TypeError("Not a jsscope scope buffer.");
		}

		this.words = words;
		this.scopeCount = words[H_SCOPE_COUNT];
		this.symbolCount = words[H_SYMBOL_COUNT];
		this.referenceCount = words[H_REFERENCE_COUNT];
		this.definitionCount = words[H_DEFINITION_COUNT];
		this.treeHandles = (words[H_FLAGS] & BUFFER_TREE_HANDLES) !== 0;
		this.scopesBase = words[H_SCOPES_BASE];
		this.symbolsBase = words[H_SYMBOLS_BASE];
		this.referencesBase = words[H_REFERENCES_BASE];
		this.definitionsBase = words[H_DEFINITIONS_BASE];
		this.poolBase = words[H_POOL_BASE];
		this.strings = new Array(words[H_STRING_COUNT]);
		this.stringOffsetsBase = words[H_STRINGS_BASE];

		const dataBase =
			(this.stringOffsetsBase + words[H_STRING_COUNT] + 1) * 4;

		this.stringBytes = new Uint8Array(
			buffer,
			dataBase,
			words[H_STRING_BYTES],
		);
	}
	...
	scopesOfNode(handle: number): number[] {
		return this.pairValues(
			this.words[H_NODE_SCOPE_BASE],
			this.words[H_NODE_SCOPE_COUNT],
			handle,
		);
	}
	...
	declaredSymbolsOfNode(handle: number): number[] {
		const lists = this.pairValues(
			this.words[H_DECLARED_BASE],
			this.words[H_DECLARED_COUNT],
			handle,
		);
	...
	referencesAtIdentifier(handle: number): number[] {
		return this.pairValues(
			this.words[H_IDENT_REF_BASE],
			this.words[H_IDENT_REF_COUNT],
			handle,
		);
	}
