 ...
 *
 * This is the compatibility path: anything that produces an ESLint-shaped AST
 * — `espree`, `@typescript-eslint/parser`, `@eslint/jsparse`'s own `toAST()` —
 * can be analyzed without going through the binary format. It is slower than
 * the binary path, because the tree it reads had to be allocated first, but it
 * runs the same walk and produces the same graph.
 *
