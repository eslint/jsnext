/**
 * @fileoverview Compares scope analysis against the analyzers jsscope aims to
 * replace.
 *
...
import {
	...
	jsxFixture,
	typescriptFixture,
} from "../../jsparse/benchmarks/fixtures.js";
...
async function contenders(dialect, code, withParse) {
	...
	if (withParse) {
		list.push({
			name: "jsparse + jsscope",
			run: () => jsscope.analyze(jsparse.parse(code), jsscopeOptions),
		});
	} else {
