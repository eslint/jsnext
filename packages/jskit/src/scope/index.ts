 ...
 *
 * `analyze()` reads the parser's binary buffers and `analyzeTree()` reads an
 * ordinary ESTree tree. They share the walk, the scope graph, and every
 * rule; all that differs is how a node is read. Both return the same
 ...
 * (the flow analyzer stores byte offsets into it) reads the layout from the
