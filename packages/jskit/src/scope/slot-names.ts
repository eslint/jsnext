 ...
 *
 * The parser addresses a node's children by slot — the position they occupy
 * in a fixed-size binary record — and `SLOT_TABLE` says which slots hold
 * children. This table says what those same children are *named*, which is
 * all a tree adapter needs to answer the same questions about an ordinary
 * ESTree node.
 *
 * The grouping below mirrors `parse/slots.ts` deliberately, so that the two
 * can be read side by side. Where that file groups kinds that
