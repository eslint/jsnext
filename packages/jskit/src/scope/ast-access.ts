 ...
 *
 * There are two ways to hand a program to this analyzer: as the binary
 * buffers the parser produces, or as an ordinary ESTree tree from any
 ...
 *   `node.type` to the same constants the parser assigns. Dispatch is then
 *   a `switch` over a small integer in both cases.
...
export interface AstAccess<TNode> {
	 ...
	 * somewhere genuinely different. The parser wraps a decorated
