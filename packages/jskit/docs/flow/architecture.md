# Control Flow Technical Specification
...
`createGraph(ast, scope)` reads the parser's binary AST and the scope
analyzer's binary scope buffer and produces a basic-block control
...
same table the scope analyzer walks with.
...
3. **Writes within a block are appended in execution order**, and emission
   ...
   meaning.
4. **Handles match the scope analyzer's binary path.** The write map is
