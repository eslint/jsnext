# Scope Analyzer Technical Specification
...
src/
  ...
  binary-ast.ts            that view over the parser's binary buffers
...
the parser's buffers, and `EstreeAst` over an ordinary tree.
...
The parser stores every node as twelve 32-bit words in one
`ArrayBuffer`, and a node is identified by its index. On that path this analyzer
...
`EstreeAst` maps `type` to the same integer constants the parser
...
`SLOT_TABLE` from the parser — the table that says, for each kind,
...
is `parse/slots.ts` with names filled in. The two files are
...
- **`TSMappedType`.** The binary format hangs the key and its constraint off a
  ...
  type.
- **A parameter's decorators.** The parser wraps a decorated parameter
...
of the source string and only calls the parser's `decodeEscapes` when a
...
When the parser gains a node kind, decide four things:
