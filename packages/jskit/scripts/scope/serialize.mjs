 ...
 *
 * The scope analyzer identifies nodes by their index in a binary buffer and
 * the two reference implementations identify them by object identity, so
 * neither shape can be compared to the other directly. Both are reduced to
 * the same thing instead: a node becomes `Type@start-end`, a variable becomes
 * its name and the scope it lives in, and a reference becomes the node it
 * occurs at.
 *
...
import { NODE_A, NODE_KIND_NAMES } from "../../dist/jskit.js";
...
/**
 * Renders a node index from a scope analysis.
 *
 ...
 * carries `espree`'s answer — that is what the parser reports in both dialects,
 ...
 * `addGlobals()` supplied to the scope analyzer has no marker, but it is
 * equally not part of the program, and nothing the program declares reaches
 * the global scope without a definition.
