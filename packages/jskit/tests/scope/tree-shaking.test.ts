...
const DIST = fileURLToPath(new URL("../../dist/jskit.js", import.meta.url));
...
describe("tree shaking", () => {
	...
	it("drops the binary reader and the parser from a tree-only bundle", async () => {
		 ...
		 */
		expect(treeOnly.code).not.toContain("Not a jskit parse buffer");
	...
	it("drops nothing either entry point actually needs", async () => {
		...
		expect(binaryOnly.code).toContain("Not a jskit parse buffer");
	});
});
